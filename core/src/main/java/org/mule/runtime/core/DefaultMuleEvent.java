...
public class DefaultMuleEvent implements MuleEvent, ThreadSafeAccess, DeserializationPostInitialisable
{
    ...
    {
        return transformMessage(DataType.builder(outputType).build());
    }
    ...
    {
        return transformMessage(DataType.builder(String.class).withEncoding(getEncoding()).build());
    }
    ...
    public String getMessageAsString(String encoding) throws MuleException
    {
        try
        {
            MuleMessage transformedMessage = getMuleContext().getTransformationService().transform(message,
                    DataType.builder(String.class).withEncoding(encoding).build());
            if (isConsumable(message.getPayload().getClass()))
            {
                setMessage(transformedMessage);
            }
