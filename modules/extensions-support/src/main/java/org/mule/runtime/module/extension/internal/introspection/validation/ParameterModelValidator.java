...
public final class ParameterModelValidator implements ModelValidator
{
    ...
    {
        ParameterGroupModelProperty parameterGroup = parameterModel
                .getModelProperty(ParameterGroupModelProperty.class)
                .orElse(null);

        if (parameterGroup != null)
        {
            if (!isInstantiable(getType(parameterModel.getType())))
            {
                throw new IllegalParameterModelDefinitionException(
                        format("The parameter group [%s] in the %s [%s] from the extension [%s] should be non abstract with a default constructor, however type '%s' was received.",
