...
public class JsonSchemaValidatorTestCase extends AbstractMuleContextTestCase
{
    ...
    private class AppleToJson extends AbstractMessageTransformer implements DiscoverableTransformer
    {
        ...
        {
            this.value = value;
            registerSourceType(DataType.builder(Apple.class).build());
            if (value instanceof InputStream)
            {
                setReturnDataType(DataType.builder(InputStream.class).build());
            }
            else
            {
                setReturnDataType(DataType.builder(value.getClass()).build());
            }
        }
