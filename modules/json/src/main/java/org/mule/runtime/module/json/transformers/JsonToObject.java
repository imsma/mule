...
{
    private static final DataType<JsonData> JSON_TYPE = DataType.builder(JsonData.class).forMimeType(MimeTypes.APPLICATION_JSON).build();

    private Map<Class<?>, Class<?>> deserializationMixins = new HashMap<Class<?>, Class<?>>();

    public JsonToObject()
    {
        this.registerSourceType(DataType.builder(Reader.class).build());
        this.registerSourceType(DataType.builder(URL.class).build());
        this.registerSourceType(DataType.builder(File.class).build());
