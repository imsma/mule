...
public class BasicTypeAutoTransformationTestCase extends AbstractMuleContextTestCase
{
    ...
    {
        return muleContext.getRegistry().lookupTransformer(DataType.STRING, DataType.builder(to).build());
    }

    private Transformer lookupToStringTransformer(Class from) throws TransformerException
    {
        return muleContext.getRegistry().lookupTransformer(DataType.builder(from).build(), DataType.STRING);
    }
