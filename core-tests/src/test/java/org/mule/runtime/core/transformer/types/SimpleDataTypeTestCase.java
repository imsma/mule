...
public class SimpleDataTypeTestCase extends AbstractMuleTestCase
{
    ...
    {
        DataType<?> dataType = DataType.builder().withEncoding(UTF_8.name()).build();
    ...
    {
        DataType.builder().withEncoding("unsupportedEncoding").build();
    }

    @Test
    public void acceptsValidMimeType() throws Exception
    {
        DataType<?> dataType = DataType.builder().forMimeType(JSON).build();
    ...
    {
        DataType.builder().forMimeType("invalidMimeType").build();
    }
}